    ...
    (function (Backbone, _) {
        ...
        function onAdd(model, collection) {
            ...
            forEachLabelInModel(model, function (label) {
                ...
                if (model[label]) collection.select(model, {
                    _silentReselect: true,
                    _externalEvent: "add",
                    label: label
                });
            });
        ...
        function onResetSingleSelect(collection, options) {
            ...
            forEachLabelInCollection(collection, function (label) {
                var removeThis = _.find(options.previousModels, function (model) {
                    return model[label];
                });
        ...
        function onResetMultiSelect(collection, options) {
            var select,
                deselect = _.filter(options.previousModels, function (model) {
                    return isModelSelectedWithAnyCollectionLabel(model, collection);
                });
            ...
            forEachLabelInCollection(collection, function (label) {
                ...
                if (select.length) _.each(select, function (model) {
                    collection.select(model, {silent: true, label: label});
                });
            });
        }
