...
from bork_api.clients import keystone_client as keystone
...
def register_pep_proxy(request, application_id, password=None):
    ...
    if not pep_proxies_group:
        LOG.error('PEP_PROXIES_GROUP is not set in settings.py')
        return

    pep_proxies_role = getattr(settings, 'PEP_PROXIES_ROLE', None)
    if not pep_proxies_role:
        LOG.error('PEP_PROXIES_ROLE is not set in settings.py')
        return
...
def get_owner_role(request, use_idm_account=True):
    owner = getattr(settings, "KEYSTONE_OWNER_ROLE")
...
def get_community_role(request, use_idm_account=True):
    community = getattr(settings, "KEYSTONE_COMMUNITY_ROLE")
    return _get_element_and_cache(
...
def get_provider_role(request, use_idm_account=True):
    provider = getattr(settings, "FIWARE_PROVIDER_ROLE_ID")
    return _get_element_and_cache(
...
def get_purchaser_role(request, use_idm_account=True):
    purchaser = getattr(settings, "FIWARE_PURCHASER_ROLE_ID")
    return _get_element_and_cache(
...
def get_default_cloud_role(request, cloud_app_id, use_idm_account=True):
    ...
    """
    default_cloud = getattr(settings, "FIWARE_DEFAULT_CLOUD_ROLE_ID")
    return _get_element_and_cache(
...
def get_idm_admin_app(request):
    idm_admin = getattr(settings, "FIWARE_IDM_ADMIN_APP", None)
...
def get_fiware_default_apps(request):
    default_apps_names = getattr(settings, "FIWARE_DEFAULT_APPS", [])
    default_apps = []
